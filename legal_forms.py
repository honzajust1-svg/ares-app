...
# Dropdown options: "145 – Společenství vlastníků jednotek"
DROPDOWN_OPTIONS: list[str] = [
    f"{kod} – {nazev}" for kod, nazev in LEGAL_FORMS.items()
]

# Vychozi: SVJ
DEFAULT_FORM = "145 – Společenství vlastníků jednotek"


def parse_dropdown_code(value: str) -> str | None:
    """Z textu '145 – Společenství...' vrátí '145', nebo None."""
    if not value or not value.strip():
        return None
    return value.strip().split(" ")[0].zfill(3)


def get_nazev(kod: str) -> str:
    """Vrátí název pro daný kód právní formy."""
    return LEGAL_FORMS.get(str(kod).zfill(3), str(kod))
